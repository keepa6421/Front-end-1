...
function getBotResponse(userMessage) {
  ...
  } else if (message.includes("about") || message.includes("who") || message.includes("background") || message.includes("education")) {
  ...
  } else if (message.includes("skill") || message.includes("ability") || message.includes("technical") || message.includes("programming")) {
    return chatbotResponses.skills[Math.floor(Math.random() * chatbotResponses.skills.length)];
  } else if (message.includes("contact") || message.includes("email") || message.includes("reach") || message.includes("message")) {
...
async function getWeather() {
  ...
  if (!city) {
    weatherInfo.innerHTML = '<p style="color: rgba(220, 53, 69, 0.9);">Please enter a city name</p>';
    return;
  }
  
  weatherInfo.innerHTML = '<p style="color: rgba(255, 255, 255, 0.7);">Loading weather data...</p>';
  ...
  try {
    ...
    setTimeout(() => {
      const demoData = {
        name: city,
        main: { temp: Math.floor(Math.random() * 30) + 10, humidity: Math.floor(Math.random() * 40) + 40 },
    ...
    weatherInfo.innerHTML = '<p style="color: rgba(220, 53, 69, 0.9);">Error fetching weather data. Please try again.</p>';
  }
}
...
async function getWeatherByLocation() {
  ...
  if (!navigator.geolocation) {
    weatherInfo.innerHTML = '<p style="color: rgba(220, 53, 69, 0.9);">Geolocation is not supported by your browser</p>';
    return;
  }
  
  weatherInfo.innerHTML = '<p style="color: rgba(255, 255, 255, 0.7);">Getting your location...</p>';
  ...
  navigator.geolocation.getCurrentPosition(
    async (position) => {
      ...
      weatherInfo.innerHTML = '<p style="color: rgba(255, 255, 255, 0.7);">Loading weather data for your location...</p>';
...
function showMyLocation() {
  ...
  if (!navigator.geolocation) {
    locationInfo.innerHTML = '<p style="color: rgba(220, 53, 69, 0.9);">Geolocation is not supported by your browser</p>';
    return;
  }
  
  locationInfo.innerHTML = '<p style="color: rgba(255, 255, 255, 0.7);">📍 Getting your location...</p>';
...
function playPause() {
  if (audioPlayer.paused) {
    ...
    playPauseBtn.style.background = "linear-gradient(135deg, rgba(220, 38, 38, 1), rgba(185, 28, 28, 1))";
  } else {
    audioPlayer.pause();
    playPauseBtn.textContent = "Play";
    playPauseBtn.style.background = "linear-gradient(135deg, rgba(220, 38, 38, 0.9), rgba(185, 28, 28, 0.9))";
  }
}
